...
echo "== ни одна функция не заканчивается голым && =="
# Функция, чья последняя команда — «[ тест ] && действие», возвращает код теста.
# Если тест ложен, функция возвращает 1, и set -e убивает скрипт молча.
# На bash 3.2 (macOS) это не воспроизводится, на bash 5.x (сервер) воспроизводится,
# поэтому проверяем статически, а не поведением.
risky="$(awk '
  /^[a-z_][a-z0-9_]*\(\) *\{/ { fn=$1; last=""; next }
  fn && /^\}/ { if (last ~ /^\[.*\] *&&/) print fn " : " last; fn=""; next }
  fn && NF && $0 !~ /^[[:space:]]*#/ { last=$0; sub(/^[[:space:]]+/,"",last) }
' "$(dirname "$0")/../hubtune.sh")"
if [ -z "$risky" ]; then PASS=$((PASS+1)); else
    FAIL=$((FAIL+1)); printf '  FAIL  функции с миной set -e:\n%s\n' "$risky"; fi
